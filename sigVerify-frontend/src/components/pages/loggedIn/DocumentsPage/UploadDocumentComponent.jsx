...
import { runDoc } from '../../../../utils/script.js';
...
const UploadDocumentComponent = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await runDoc();
            console.log(res);
            setDocResult(res);  // Save result to state
            setIsSubmitSuccess(true);  // Indicate successful submission
        } catch (error) {
            console.error('Failed to process document:', error);
            setIsSubmitSuccess(false);  // Optionally handle failure
        }
    };
            ...
            <UploadDocumentContainer>
                        ...
                        <DocumentForm onSubmit={handleSubmit}>
                            ...
                            <div>
                                ...
                                <input
                                    type="text"
                                    name="recipients1"
                                    value={customFormData.recipients1}
                                    onChange={handleChange}
                                />
                            </div>
                            {customFormData.recipients1.toLowerCase() === "david@commons.com" && (
                                <span style={{ marginTop: '5px', color: 'green', display: 'flex', paddingLeft: '150px' }}>
                                    ☑️ Commons HQ <br /> ✅ USA ID Verified <br /> ✅ Email Verified
                                </span>
                            )}
                            <button className="buttonPop">
                                Submit
                            </button>
                        </DocumentForm>
                    </UploadFileBox>
                ...
                {!isSubmitSuccess ? (
                    ...
                    <div style={{ padding: '20px', textAlign: 'center' }}>
                        <h1>Success</h1>
                        <p>Your document has been successfully created and signed by yourself!</p>
                        <p>The document has been sent to the recipients emails</p>
                        <p>The document ID on-chain : </p>
                        <a href={docResult}>XRPL Explorer</a>
